...
import net.sf.mzmine.modules.visualization.kendrickmassplot.chartutils.KendrickMassPlotMobilogramListToolTipGenerator;
...
public class KendrickMassPlotTask extends AbstractTask {
  ...
  private MobilogramListRow rows[];
  private ParameterSet parameterSet;
  private int totalSteps = 3, appliedSteps = 0;

  public KendrickMassPlotTask(ParameterSet parameters) {
    mobilogramList = parameters.getParameter(KendrickMassPlotParameters.mobilogramList).getValue()
        .getMatchingMobilogramLists()[0];

    title = "Kendrick mass plot [" + mobilogramList + "]";
    ...
    rows = parameters.getParameter(KendrickMassPlotParameters.selectedRows)
  ...
  public String getTaskDescription() {
    return "Create Kendrick mass plot for " + mobilogramList;
  }
  ...
  public void run() {
    ...
    logger.info("Create Kendrick mass plot of " + mobilogramList);
    // Task canceled?
    if (isCanceled())
      return;
    ...
    logger.info("Finished creating Kendrick mass plot of " + mobilogramList);
  }
  ...
  private JFreeChart create2DKendrickMassPlot() {
    ...
    if (zAxisLabel.equals("none")) {
      ...
      KendrickMassPlotMobilogramListToolTipGenerator tooltipGenerator =
          new KendrickMassPlotMobilogramListToolTipGenerator(xAxisLabel, yAxisLabel, zAxisLabel,
              rows);
