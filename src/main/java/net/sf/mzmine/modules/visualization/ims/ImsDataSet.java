...
class ImsDataSet extends AbstractXYDataset implements Task {
  ...
  private final Range<Double> totalRTRange;
  ...
  ImsDataSet(RawDataFile rawDataFile, Scan scans[], Range<Double> rtRange, Range<Double> mzRange,
      ImsVisualizerWindow visualizer) {
  ...
  public void run() {
    ...
    for (int index = 0; index < totalScans; index++) {
      ...
      driftTime = getDriftTimeBinNumber(scan);
  ...
  private int getDriftTimeBinNumber(Scan scan) {
    int driftTime = 0;
    ArrayList<Integer> scanNumbers = new ArrayList<Integer>();
    int scanNumber = scan.getScanNumber();
    double scanRetentionTime = scan.getRetentionTime();
    for (int i = 0; i < scans.length; i++) {
      if (scanRetentionTime == scans[i].getRetentionTime()) {
        scanNumbers.add(scans[i].getScanNumber());
      }
    }
    driftTime = scanNumber - scanNumbers.get(0);
    return driftTime;
  }
  ...
  public String getTaskDescription() {
    return "Updating IMS visualizer of " + rawDataFile;
  }
