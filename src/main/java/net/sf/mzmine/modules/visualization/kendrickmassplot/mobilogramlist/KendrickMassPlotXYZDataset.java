  ...
  public KendrickMassPlotXYZDataset(ParameterSet parameters) {
    ...
    if (parameters.getParameter(KendrickMassPlotParameters.zAxisCustomKendrickMassBase)
      ...
      for (int i = 0; i < selectedRows.length; i++) {
        ...
        if (zAxisKMBase.equals("Retention time")) {
        ...
        } else if (zAxisKMBase.equals("Mobility")) {
          zValues[i] = selectedRows[i].getAverageMobility();
          ...
          zValues[i] = selectedRows[i].getBestMobilogram().getTailingFactor();
        } else if (zAxisKMBase.equals("Asymmetry factor")) {
          zValues[i] = selectedRows[i].getBestMobilogram().getAsymmetryFactor();
        } else if (zAxisKMBase.equals("FWHM")) {
          zValues[i] = selectedRows[i].getBestMobilogram().getFWHM();
        } else if (zAxisKMBase.equals("m/z")) {
          zValues[i] = selectedRows[i].getBestMobilogram().getMZ();
        }
      }
  }
