...
import net.sf.mzmine.modules.visualization.mobilogramlisttable.ColumnSettingParameter;
 ...
 * 
 */
public class MobilogramListTableColumnModel extends DefaultTableColumnModel
  ...
  private FormattedCellRenderer mzRenderer, rtRenderer, mobilityRenderer, ccsRenderer,
      intensityRenderer;
  private TableCellRenderer mobilogramShapeRenderer, identityRenderer, mobilogramStatusRenderer,
   ...
   * 
   */
  MobilogramListTableColumnModel(GroupableTableHeader header, MobilogramListTableModel tableModel,
      ParameterSet parameters, MobilogramList mobilogramList) {
    ...
    mobilityRenderer = new FormattedCellRenderer(rtFormat);
    ccsRenderer = new FormattedCellRenderer(rtFormat);
    intensityRenderer = new FormattedCellRenderer(intensityFormat);
    mobilogramShapeRenderer = new MobilogramShapeCellRenderer(mobilogramList, parameters);
    identityRenderer = new CompoundIdentityCellRenderer();
    mobilogramStatusRenderer = new MobilogramStatusCellRenderer();
  ...
  public void createColumns() {
    ...
    ColumnSettingParameter<CommonColumnType> csPar =
        parameters.getParameter(MobilogramListTableParameters.commonColumns);
    CommonColumnType visibleCommonColumns[] = csPar.getValue();

    // This is a workaround for a bug - we need to always show the ID, m/z
    // and RT columns, otherwise manual editing of mobilogram identities does not
    // work.
    ...
    commonColumnsList.remove(CommonColumnType.AVERAGEMOBILITY);
    commonColumnsList.remove(CommonColumnType.AVERAGECCS);
    ...
    commonColumnsList.add(3, CommonColumnType.AVERAGEMOBILITY);
    commonColumnsList.add(4, CommonColumnType.AVERAGECCS);

    visibleCommonColumns = commonColumnsList.toArray(visibleCommonColumns);

    ColumnSettingParameter<DataFileColumnType> dfPar =
        parameters.getParameter(MobilogramListTableParameters.dataFileColumns);
    ...
    for (int i = 0; i < visibleCommonColumns.length; i++) {
      ...
      switch (commonColumn) {
        ...
        case AVERAGEMOBILITY:
          newColumn.setCellRenderer(mobilityRenderer);
          break;
        case AVERAGECCS:
          newColumn.setCellRenderer(ccsRenderer);
          break;
        case IDENTITY:
        ...
        case COMMENT:
          ...
          break;
        case PEAKSHAPE:
          newColumn.setCellRenderer(mobilogramShapeRenderer);
          break;
        default:
      ...
      if ((commonColumn == CommonColumnType.AVERAGEMZ)
          ...
          || (commonColumn == CommonColumnType.AVERAGEMOBILITY)
          || (commonColumn == CommonColumnType.AVERAGECCS)) {
        averageGroup.add(newColumn);
      }

    }

    for (int i = 0; i < mobilogramList.getNumberOfRawDataFiles(); i++) {

      RawDataFile dataFile = mobilogramList.getRawDataFile(i);
      ...
      for (int j = 0; j < visibleDataFileColumns.length; j++) {
        ...
        switch (dataFileColumn) {
          ...
          case PEAKSHAPE:
            ...
            break;
          case STATUS:
            newColumn.setCellRenderer(mobilogramStatusRenderer);
            break;
          case RT:
            newColumn.setCellRenderer(rtRenderer);
            break;
  ...
  public void mouseReleased(MouseEvent e) {
    ...
    if (modelIndex < numOfCommonColumns) {
      ...
      for (int dataFileIndex =
          mobilogramList.getNumberOfRawDataFiles() - 1; dataFileIndex >= 0; dataFileIndex--) {
        int columnIndex =
