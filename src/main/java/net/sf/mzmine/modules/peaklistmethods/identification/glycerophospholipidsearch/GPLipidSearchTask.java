	...
	public void run() {
		...
		// Calculate how many possible lipids we will try
				for (int i = 0; i < selectedLipids.length; i++) {
					double stepsPerLipid = Math.pow(((maxChainLength + 1)* (maxDoubleBonds + 1)), selectedLipids[i].getNumberOfChains());
					totalSteps = totalSteps + stepsPerLipid;
				}
						...
						if(lipidType.getNumberOfChains() >= 2) {
							for (int fattyAcid2Length = 0; fattyAcid2Length <= maxChainLength; fattyAcid2Length++) {
								for (int fattyAcid2DoubleBonds = 0; fattyAcid2DoubleBonds <= maxDoubleBonds; fattyAcid2DoubleBonds++) {
									...
									if(lipidType.getNumberOfChains() == 2) {
										// Prepare a lipid instance
										GPLipidIdentity2Chains lipid2Chains = new GPLipidIdentity2Chains(
												lipidType, fattyAcid1Length,
												fattyAcid1DoubleBonds, fattyAcid2Length,
												fattyAcid2DoubleBonds);

										// Find all rows that match this lipid
										findPossibleGPL2Chains(lipid2Chains, rows);

										finishedSteps++;
									}
									if(lipidType.getNumberOfChains() >= 3) {
										for (int fattyAcid3Length = 0; fattyAcid3Length <= maxChainLength; fattyAcid3Length++) {
											for (int fattyAcid3DoubleBonds = 0; fattyAcid3DoubleBonds <= maxDoubleBonds; fattyAcid3DoubleBonds++) {
												...
												if (((fattyAcid1Length > 0) && (fattyAcid1Length < minChainLength))
														|| ((fattyAcid2Length > 0) && (fattyAcid2Length < minChainLength))
														|| ((fattyAcid3Length > 0) && (fattyAcid3Length < minChainLength))) {
													finishedSteps++;
													continue;
												}
												...
												if (((fattyAcid1DoubleBonds > 0) && (fattyAcid1DoubleBonds > fattyAcid1Length - 1))
														|| ((fattyAcid2DoubleBonds > 0) && (fattyAcid2DoubleBonds > fattyAcid2Length - 1))
														|| ((fattyAcid3DoubleBonds > 0) && (fattyAcid3DoubleBonds > fattyAcid3Length - 1))) {
													finishedSteps++;
													continue;
												}
												if(lipidType.getNumberOfChains() == 3) {
													// Prepare a lipid instance
													GPLipidIdentity3Chains  lipid3Chains = new GPLipidIdentity3Chains(
															lipidType, fattyAcid1Length,
															fattyAcid1DoubleBonds, fattyAcid2Length,
													...
													// Find all rows that match this lipid
													findPossibleGPL3Chains(lipid3Chains, rows);

													finishedSteps++;
												}
												if(lipidType.getNumberOfChains() >= 4) {
													for (int fattyAcid4Length = 0; fattyAcid4Length <= maxChainLength; fattyAcid4Length++) {
														for (int fattyAcid4DoubleBonds = 0; fattyAcid4DoubleBonds <= maxDoubleBonds; fattyAcid4DoubleBonds++) {
															...
															if (((fattyAcid1Length > 0) && (fattyAcid1Length < minChainLength))
																	|| ((fattyAcid2Length > 0) && (fattyAcid2Length < minChainLength))
																	|| ((fattyAcid3Length > 0) && (fattyAcid3Length < minChainLength))
																	|| ((fattyAcid4Length > 0) && (fattyAcid4Length < minChainLength))) {
																finishedSteps++;
																continue;
															}
															...
															if (((fattyAcid1DoubleBonds > 0) && (fattyAcid1DoubleBonds > fattyAcid1Length - 1))
																	|| ((fattyAcid2DoubleBonds > 0) && (fattyAcid2DoubleBonds > fattyAcid2Length - 1))
																	|| ((fattyAcid3DoubleBonds > 0) && (fattyAcid3DoubleBonds > fattyAcid3Length - 1))
																	|| ((fattyAcid4DoubleBonds > 0) && (fattyAcid4DoubleBonds > fattyAcid4Length - 1))) {
																finishedSteps++;
																continue;
															}
															if(lipidType.getNumberOfChains() == 4) {
																// Prepare a lipid instance
																GPLipidIdentity4Chains  lipid4Chains = new GPLipidIdentity4Chains(
																		lipidType, fattyAcid1Length,
																		fattyAcid1DoubleBonds, fattyAcid2Length,
																		fattyAcid2DoubleBonds,fattyAcid3Length,
																		fattyAcid3DoubleBonds,fattyAcid4Length,
																		fattyAcid4DoubleBonds);

																// Find all rows that match this lipid
																findPossibleGPL4Chains(lipid4Chains, rows);
